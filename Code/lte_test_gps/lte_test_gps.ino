...
void printMenu(void) {
  ...
  Serial.println(F("[w] GET"));
  Serial.println(F("[W] POST"));
...
//        float latitude, longitude, speed_kph, heading, altitude, second;
    ...
    case 'w': {
        ...
        sprintf(url,"easystroll.appspot.com/gps?lat=%s&lon=%s",lat,lon);
        Serial.println(url);
        ...
        int16_t counter = length;
        while (length > 0) {
          while (fona.available()) {
        //    char c = fona.read();
              buf[counter - length] = fona.read();
